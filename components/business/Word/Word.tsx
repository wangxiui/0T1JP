...
interface Props {
  item: {jp: string, cn: string, spell: string} // item数据
...
export default function Word(props: Props) {
  ...
  const rootClassName = cn(
    ...
    flex-col rounded-lg shadow-lg shadow-blue-500/50 text-7xl`,
    className
  );
  ...
  return (
    ...
    isPositive
      ...
      : (
        <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className={rootClassName} onClick={() => setIsPositive(true)}>
          <div>{item.jp}</div>
          <div className='mt-6 text-gray-300 text-5xl'>{item.spell}</div>
        </motion.div>
      )
  )
}
